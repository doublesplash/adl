...
try {
    ...
    while (count($argv) > 0) {
        ...
        } else if (trim($arg) != '/home/rdm/init/upgrade.sh') {
    ...
    // Start Output
    if (!$is_silent) {
        echo StringTools::consoleColor('Upgrade Script', StringTools::CONSOLE_COLOR_GREEN) . "\n";
    ...
    $current_version = '0';
    try {
        $setting = new \Adl\Setting();
        $ret_val = $setting->query(array('name' => 'version'), false);
        
        // If we don't have a version yet, the save a default one
        if (is_null($ret_val)) {
            $setting->setName('version');
            $setting->setValue('20000101');
            $setting->insert();
        }
        
        $current_version = $setting->getValue();
    ...
    $revision_count = 0;
    $version_folders = scandir($webapp_dir . '/lib/Adl/Migration/');
    foreach ($version_folders as $version_folder) {
        if (strpos($version_folder, '.') === 0) { continue; }
        
        if (file_exists($webapp_dir . '/lib/Adl/Migration/' . $version_folder)) {
            if (!$is_silent && $debug) { StringTools::consoleWrite(' - Scanning /lib/Adl/Migration/' . $version_folder, null, StringTools::CONSOLE_COLOR_YELLOW, true); }
            ...
            if (($migration_revision > $current_version) || ($migration_revision == $arg_version) || ($migrate_greater_versions && ($migration_revision > $arg_version))) {
                $revision_files = scandir($webapp_dir . '/lib/Adl/Migration/' . $version_folder);
                foreach ($revision_files as $revision_file) {
                    if (strpos($revision_file, '.') === 0) { continue; }
                    if (strpos($revision_file, '.php') === false) { continue; }
                    if (!$is_silent && $debug) { StringTools::consoleWrite('   - Adding /lib/Adl/Migration/' . $version_folder . '/' . $revision_file, null, StringTools::CONSOLE_COLOR_YELLOW, true); }
                    $revision_array[str_replace('rev', '', $migration_revision)][] = array(
                            'file' => $webapp_dir . '/lib/Adl/Migration/' . $version_folder . '/' . $revision_file,
    ...
    if (count($revision_array) == 0) {
        ...
        foreach ($revision_array as $migration_version => $revision_files) {
            foreach ($revision_files as $revision_array) {
                ...
                $class_name = 'Adl\\Migration\\' . $migration_revision . '\\' . $class_name;
        ...
        try {
            $setting->setValue($new_version);
            $ret_val = $setting->update();
