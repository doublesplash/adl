...
class Connection {
	...
	static function loadDatabasesFromFile($filename) {
		...
		foreach ($ini_file['databases'] as $key => $value) {
			if (isset($ini_file[$value])) {
				$db_array[$key]['user'] = isset($ini_file[$value]['param.user']) ? $ini_file[$value]['param.user'] : '';
				$db_array[$key]['pass'] = isset($ini_file[$value]['param.password']) ? $ini_file[$value]['param.password'] : '';
				$db_array[$key]['database'] = isset($ini_file[$value]['param.database']) ? $ini_file[$value]['param.database'] : '';
				$db_array[$key]['host'] = isset($ini_file[$value]['param.host']) ? $ini_file[$value]['param.host'] : '';
				$db_array[$key]['port'] = isset($ini_file[$value]['param.port']) ? $ini_file[$value]['param.port'] : '';
	...
	function getDbConnection($name) {
		...
		if (($res = mysql_connect($db_array['host'], $db_array['user'], $db_array['pass'])) !== null) {
