...
class Ajax extends MojaviObject {
    ...
    private function curl($orig_url, $params = array(), $method = Request::GET, $user_token = "", $api_token = "", $remove_unsafe_params = true, $retry_count = 0) {
        try {
            ...
            $http_client->setHeaders(array('Accept-Encoding: gzip,deflate'));

            if ($http_client->getRequest()->getUri() === null) {
            ...
            if ($method == \Mojavi\Request\Request::GET) {
                $http_client->setMethod(\Zend\Http\Request::METHOD_GET);
            	if (is_array($params)) { $http_client->setParameterGet($params); }
                $response = $http_client->send();
            } else if ($method == \Mojavi\Request\Request::POST) {
            	$http_client->setMethod(\Zend\Http\Request::METHOD_POST);
                // If we uploaded files, we have to send them differently
                if (count($_FILES) > 0) {
                	foreach ($_FILES as $key => $value) {
                		$http_client->setFileUpload($value['tmp_name'], $key);
                	}
                }
                if (is_array($params)) { $http_client->setParameterPost($params); }
                $response = $http_client->send();
            } else if ($method == \Mojavi\Request\Request::DELETE) {
                $http_client->setMethod(\Zend\Http\Request::METHOD_DELETE);
            	if (is_array($params)) { $http_client->setParameterGet($params); }
                $response = $http_client->send();
            } else if ($method == \Mojavi\Request\Request::PUT) {
                $http_client->setMethod(\Zend\Http\Request::METHOD_PUT);
            	if (count($_FILES) > 0) {
                	foreach ($_FILES as $key => $value) {
                		$http_client->setFileUpload($value['tmp_name'], $key);
                	}
